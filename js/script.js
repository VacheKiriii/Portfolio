...
function initTheme() {
    ...
    if (!document.querySelector('.theme-toggle')) {
        ...
        themeToggle.innerHTML = `<i class="${savedTheme === 'dark' ? 'fas fa-sun' : 'fas fa-moon'}"></i>`;
        themeToggle.setAttribute('aria-label', `Passer au thème ${savedTheme === 'dark' ? 'clair' : 'sombre'}`);
        themeToggle.setAttribute('title', `Passer au thème ${savedTheme === 'dark' ? 'clair' : 'sombre'}`);
        themeToggle.addEventListener('click', toggleTheme);
        document.body.appendChild(themeToggle);
    }
    
    // Ajout du fichier CSS theme.css s'il n'existe pas déjà
    if (!document.querySelector('link[href*="theme.css"]')) {
        const themeLink = document.createElement('link');
        themeLink.rel = 'stylesheet';
        themeLink.href = window.location.pathname.includes('/templates/') ? '../css/theme.css' : 'css/theme.css';
...
// Fonction spécifique aux popups de la page réalisation
function openPopupRealisation() {
    openPopup('popup-realisation');
}

function closePopupRealisation() {
    closePopup('popup-realisation');
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Ajouter un effet de zoom sur les images dans les popups
    const popupImages = document.querySelectorAll('.popup-images img');
